...
print(f'Raw bytes: {raw5.hex()} state[0x2c]={combined:#010x}')
...
def hook(uc, address, size, user_data):
    ...
    if address == 0xcf0c2c:
        ...
        t = 'TAKEN' if w2 <= w1 else 'FALL'
        print(f'  [{inctr[0]}] 0xcf0c2c {t} w2={w2:#x} w1={w1:#x} w15={w15:#x} w0={w0:#x} x14={x14:#x}')
    elif address == 0xcf0e00:
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        w1 = uc.reg_read(UC_ARM64_REG_W1)
        print(f'  [{inctr[0]}] 0xcf0e00 DECODE w0={w0:#x} w1={w1:#x}')
    elif address == 0xcf0e44:
        w14 = uc.reg_read(UC_ARM64_REG_W14)
        w1 = uc.reg_read(UC_ARM64_REG_W1)
        t = 'TAKEN' if w14 <= w1 else 'FALL'
        print(f'  [{inctr[0]}] 0xcf0e44 {t} w14={w14:#x} w1={w1:#x}')
    elif address == 0xcf1408:
        w9 = uc.reg_read(UC_ARM64_REG_W9)
        w22 = uc.reg_read(UC_ARM64_REG_W22)
        print(f'  [{inctr[0]}] 0xcf1408 SHORT output_pos={struct.unpack("<Q", mu.mem_read(STATE_ADDR+0x30, 8))[0]:#x} state40={w9:#x} state44={w22:#x}')
    elif address == 0xcf141c:
        w4 = uc.reg_read(UC_ARM64_REG_W4)
        print(f'  [{inctr[0]}] 0xcf141c cbz w4={w4:#x} -> {"ERR" if w4==0 else "OK"}')
    elif address == 0xcf0e48:
        w15 = uc.reg_read(UC_ARM64_REG_W15)
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        w1 = uc.reg_read(UC_ARM64_REG_W1)
        output_pos = struct.unpack("<Q", mu.mem_read(STATE_ADDR+0x30, 8))[0]
        print(f'  [{inctr[0]}] 0xcf0e48 w15={w15:#x} w0={w0:#x} w1={w1:#x} out_pos={output_pos:#x}')
    elif address == 0xcf1774:
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        sp = uc.reg_read(UC_ARM64_REG_SP)
        x29 = uc.reg_read(UC_ARM64_REG_X29)
        lr = uc.reg_read(UC_ARM64_REG_X30)
        print(f'  [{inctr[0]}] 0xcf1774 ERR w0={w0:#x} SP={sp:#x} X29={x29:#x} LR={lr:#x}')
    elif address == 0xcf1788:
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        print(f'  [{inctr[0]}] 0xcf1788 RET w0={w0:#x}')
    elif address == 0xcf0b04:
        x0 = uc.reg_read(UC_ARM64_REG_X0)
        x1 = uc.reg_read(UC_ARM64_REG_X1)
        x2 = uc.reg_read(UC_ARM64_REG_X2)
        output_pos = struct.unpack("<Q", mu.mem_read(STATE_ADDR+0x30, 8))[0]
        print(f'\n=== sub_CF0B04 CALL x0={x0:#x} x1={x1:#x} x2={x2:#x} out_pos={output_pos:#x} ===')
