...
tests = [
    ('standard', False, False, lambda si,bc,mk: (si<<4)+(bc&mk)),
    ('bc_before', True, False, lambda si,bc,mk: (si<<4)+(bc&mk)),
    ('bc_match', False, True, lambda si,bc,mk: (si<<4)+(bc&mk)),
    ('no_bc', False, False, lambda si,bc,mk: (si<<4)),
]

for label,bc_before,bc_match,fn in tests:
    ...
    try:
        for sn in range(2000):
            ...
            if b==0:
                ...
                if not bc_before and not bc_match: bc+=1
                for i2 in range(l2):
                    ...
                    si=ctx[0]&0xF
        m=sum(1 for a,b in zip(out[:12],exp) if a==b) if len(out)>=12 else 0
        print(f'{label:>12}: match={m}/12 size={len(out):5d} first={out[:24].hex()}')
    except Exception as ex2:
        print(f'{label:>12}: ERROR {ex2}')
