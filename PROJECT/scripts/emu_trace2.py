...
def hook(uc, address, size, user_data):
    ...
    if address == 0xcf0c2c:
        ...
        print(f'  [{inctr[0]}] 0xcf0c2c b.ls w2={w2:#x} w1={w1:#x} w15={w15:#x} w0={w0:#x} x14={x14:#x}', end='')
        print(' TAKEN' if w2 <= w1 else ' FALL-THROUGH')
    elif address == 0xcf0e00:
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        w1 = uc.reg_read(UC_ARM64_REG_W1)
        w15 = uc.reg_read(UC_ARM64_REG_W15)
        print(f'  [{inctr[0]}] 0xcf0e00 MAIN DECODE w0={w0:#x} w1={w1:#x}')
    elif address == 0xcf0e44:
        w14 = uc.reg_read(UC_ARM64_REG_W14)
        w1 = uc.reg_read(UC_ARM64_REG_W1)
        print(f'  [{inctr[0]}] 0xcf0e44 b.ls w14={w14:#x} w1={w1:#x}', end='')
        print(' TAKEN' if w14 <= w1 else ' FALL-THROUGH')
    elif address == 0xcf1408:
        w9 = uc.reg_read(UC_ARM64_REG_W9)
        w22 = uc.reg_read(UC_ARM64_REG_W22)
        print(f'  [{inctr[0]}] 0xcf1408 SHORT PATH w9(40)={w9:#x} w22(44)={w22:#x}')
    elif address == 0xcf141c:
        w4 = uc.reg_read(UC_ARM64_REG_W4)
        print(f'  [{inctr[0]}] 0xcf141c cbz w4={w4:#x} -> {"ERROR" if w4==0 else "OK"}')
    elif address == 0xcf1774:
        print(f'  [{inctr[0]}] 0xcf1774 ERROR PATH ENTERED!')
    elif address == 0xcf1788:
        w0 = uc.reg_read(UC_ARM64_REG_W0)
        print(f'  [{inctr[0]}] 0xcf1788 mov w0=1, ERROR RET')
