...
def hook(uc, address, size, user_data):
    ...
    # Trace ALL instructions in the range 0xcf24e0-0xcf25a0
    if 0xcf24e0 <= address <= 0xcf25a0 or address == 0xcf1a44:
        if inctr[0] > 5740:
            print(f'  [{inctr[0]}] 0x{address:#x}')
            if address == 0xcf1a44:
                print('  *** SUB_CF1A44 REACHED ***')
